        ...
        {courses.map((c) => (
          <div key={c.id} className="rounded-xl border border-gray-100 p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-bold text-brand-navy">{c.title} <span className="ml-2 rounded bg-gray-100 px-2 py-0.5 text-xs capitalize">{c.status}</span></p>
                <p className="text-xs text-gray-400">{c.creator_email}</p>
              </div>
              <a href={`/courses/${c.slug}`} target="_blank" rel="noopener noreferrer" className="text-sm text-brand-blue hover:underline">Preview ↗</a>
            </div>
            {(ACTIONS[c.status] ?? []).length > 0 && (
              <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center">
                {ACTIONS[c.status].includes("reject") && (
                  <input placeholder="Reject reason" value={reason[c.id] ?? ""} onChange={(e) => setReason({ ...reason, [c.id]: e.target.value })} className="flex-1 rounded-md border border-gray-200 px-3 py-1.5 text-sm" />
                )}
                {ACTIONS[c.status].map((a) => (
                  <button key={a} onClick={() => act(c.id, a)} className="rounded-md bg-brand-blue px-4 py-1.5 text-sm font-medium capitalize text-white hover:opacity-90">{a}</button>
                ))}
              </div>
            )}
          </div>
